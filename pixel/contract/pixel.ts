...
// 自己发行能量
// 众筹合约类
export default class PixelGameContract extends AschContract {
  ...
  buyCoins(amount: number, currency: string): void {
    ...
    if (this.playersInfo[senderId] === undefined) {
      this.playersInfo[senderId] = { holding: 0, ownPixelNum: 0, bonusMask: 0, bonusSettled: 0, bonusCollected: 0 }
    }
  ...
  getPixelMapInfo(position: number): PixelInfo {
    if (this.map[String(position)] === undefined) {
      return { owner: '', price: 50, color: 0, version: 0 }
    }
